...
ifndef TOXIC_BUILD_TYPE
    TOXIC_BUILD_TYPE := Alpha
    PLATFORM_VERSION_CODENAME := Alpha
endif
TOXIC_VER := 0.5
 ifdef TOXIC_BUILD_TYPE
     TOXIC_VERSION_NUMBER = $(TOXIC_VER)-$(TOXIC_BUILD_TYPE)
 else
     TOXIC_VERSION_NUMBER = $(TOXIC_VER)
 endif

# Set all versions
TOXIC_VERSION := Toxic-OS_$(TOXIC_BUILD)-v$(ANDROID_VERSION)_$(shell date +%Y%m%d)_$(TOXIC_VERSION_NUMBER)
TOXIC_MOD_VERSION := Toxic-OS_$(TOXIC_BUILD)-v$(ANDROID_VERSION)_$(shell date +%Y%m%d)_$(TOXIC_VERSION_NUMBER)

PRODUCT_PROPERTY_OVERRIDES += \
    BUILD_DISPLAY_ID=$(BUILD_ID) \
    ro.toxic.version=$(TOXIC_VERSION_NUMBER) \
    ro.mod.version=$(TOXIC_BUILD_TYPE)-v$(TOXIC_VERSION_NUMBER) \
